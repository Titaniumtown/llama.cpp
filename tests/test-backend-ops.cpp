...
// GGML_OP_L2_NORM x N (independent siblings in one graph)
// A backend may batch adjacent same-shape L2_NORMs into a single launch. Nothing above
// can reach that path: every other case runs one op per graph. This mirrors the shape it
// exists for -- a gated-delta-net's q/k norms, which are STRIDED views carved out of the
// fused qkv buffer, not contiguous tensors -- and nests the reduction to the right so all
// N norms land adjacent in the graph rather than interleaved with the adds.
struct test_l2_norm_batch : public test_case {
    ...
    ggml_tensor * build_graph(ggml_context * ctx) override {
        ...
        if (strided) {
            // slices along dim1 leave each view with a dim2 stride that is n_norms times
            // the contiguous one, so ggml_is_contiguous() is false for every source
            parent = ggml_new_tensor_4d(ctx, type, ne[0], ne[1] * n_norms, ne[2], ne[3]);
            ggml_set_name(parent, "qkv");
        }
        ...
        for (int t = 0; t < n_norms; ++t) {
            ...
            ggml_set_name(norms[t], ("norm" + std::to_string(t)).c_str());
        }
...
static std::vector<std::unique_ptr<test_case>> make_test_cases_eval() {
    ...
    // IMROPE was the one mode this sweep did not carry, and it is what Qwen3/Qwen3.5/
    // Qwen3TTS run -- so the rope+set_rows fusion was untested on the family most likely
    // to exercise it. It shares rope_multi() with MROPE and differs only in how the
    // section index picks a position, which is orthogonal to the fused write.
    for (int mode : { GGML_ROPE_TYPE_NORMAL, GGML_ROPE_TYPE_NEOX, GGML_ROPE_TYPE_MROPE,
                      GGML_ROPE_TYPE_IMROPE, GGML_ROPE_TYPE_VISION }) {
        for (ggml_type type : {GGML_TYPE_F16, GGML_TYPE_F32}) {
            for (int ne2 : {1, 8, 512}) {
...
// Conv_2D test cases
 ...
 // A deep layer of a ConvNet, several images in the batch
    ...
    // chunked-path coverage at S_v=128 (production shape) and with every dispatch
    // feature crossed with a multi-chunk length: broadcast heads + multi-seq, K>1
    // snapshots (tail keeps them), folded sigmoid + folded state gather.
    test_cases.emplace_back(new test_gated_delta_net(GGML_TYPE_F32, 32, 128, 256, 1));
    test_cases.emplace_back(new test_gated_delta_net(GGML_TYPE_F32, 8, 128, 200, 2, 2));
    test_cases.emplace_back(new test_gated_delta_net(GGML_TYPE_F32, 4, 64, 200, 1, 1, false, false, /*K=*/5));
    test_cases.emplace_back(new test_gated_delta_net(GGML_TYPE_F32, 4, 128, 192, 1, 1, false, false, /*K=*/1, /*beta_sigmoid=*/true, /*state_rows=*/4));
